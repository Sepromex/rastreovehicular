...
function otro_server($lat,$lon,$ide)
{
	//calcula el cruce de las calles
    //$conec2 = @mysql_connect("160.16.18.8","supervisor","supervisor");
	...
	$conec2 = @mysql_connect("10.0.2.13:3306","usercruce","server");
...
switch($accion)
{
	case 'folios'://ver los folios de la empresa
		...
		$correos=mysql_query("SELECT * from correos_empresa where id_empresa=".$_GET['empresa']." and activo=1",$conec);
		$i=0;
	...
	case 'regla_c'://inserto la nuevo "regla"
		...
		*/
		...
		for($i=0;$i<count($vehiculos);$i++){
			...
			mysql_query($insert_detalle,$conec);
	...
	case 'insertar_c':
		...
		$ver=mysql_query("select * from correos_empresa where id_empresa=".$_GET['empresa']." AND correo like '$correo'",$conec);
		if(mysql_num_rows($ver)==0){
			$nombre=str_replace("_"," ",$nombre);
			$insert_c="INSERT INTO correos_empresa values(".$_GET['empresa'].",0,'$nombre','$correo',1)";
			mysql_query($insert_c,$conec);
		}
		else{
			mysql_query("UPDATE correos_empresa set activo=1,nombre='$nombre' 
			where id_empresa=".$_GET['empresa']." and correo='$correo'",$conec);
		}
		if(!mysql_error()){
	...
	case 'geocercas_c': //consulto las geocercas Circulares
		...
		and id_usuario=".$_GET['Usuario']." and activo=1",$conec);
		$i=0;
		$cadena="";
	...
	case 'insertar_g': //inserto una nueva geocerca Circulares
		$id_us=mysql_query("select id_usuario from usuarios where username='".$_GET['Usuario']."'    
		  and id_empresa=".$_GET['empresa'],$conec);
		...
		if($idu!=''){
			...
			mysql_query("INSERT INTO geo_time values(0,$lat,$lon,$radio,0,'$nombre','$idu',0,".$_GET['empresa'].",1)",$conec);
			if(!mysql_error()){
	...
	break;
	case 'gpsdetalle':
		$detalles=mysql_query("select * from gpscondicionalertadet where folio=".$_GET['folio'],$conec);
		$i=0;
		$cadena="";
	...
	break;
	case 'geocercas_d':
		mysql_query("UPDATE geo_time set activo=0 where num_geo=".$_GET['folio'],$conec);
		if(!mysql_error()){
			echo "OK";
		}
		else{
	...
	case 'geocercas_u';
		...
		where num_geo=".$_GET['folio'],$conec);
		if(!mysql_error()){
			echo "OK";
		}
		else{
	...
	case 'detalle':
		$folio=$_GET['Folio'];
		$ver=mysql_query("SELECT * from gpscondgeocompleja where folio_entgeo=$folio",$conec);
		$cadena="";
		if(mysql_num_rows($ver)>0){
			...
			$query=mysql_query("SELECT * from gpscondicionalertadet where folio=$folio and activo=1",$conec);
			while($row=mysql_fetch_array($query)){
	...
	*/
	case 'login':  //Control de acceso al sistema 
		$user=$_GET['usuario'];
		$pass=$_GET['password'];
		$ver_usuario=mysql_query("SELECT * from usuarios where BINARY username ='".$user."' ",$conec);
		if(mysql_num_rows($ver_usuario)>0){
			$ver_pass=mysql_query("SELECT * from usuarios where BINARY username ='$user' and password='$pass'",$conec);
			if(mysql_num_rows($ver_pass)>0){
				$fechas=mysql_query("SELECT * from usuarios where BINARY username ='$user' and password='$pass' 
...
/*$q_v="SELECT distinct(v.num_veh),v.id_veh,v.placas,(u.lat/3600/16), ((u.long & 8388607)/3600/12*-1), u.fecha, u.velocidad, 
		if (SUBSTRING(s.descripcion,4,1) ="U", u.odometro/1000, u.odometro) as odometro, u.satelites
		from vehiculos v
		...
		if(mysql_num_rows($vehiculos)>0){
			...
			while($row=mysql_fetch_array($vehiculos)){
				...
				echo $row[0]."|".$row[1]."|".$row[2]."|".$row[3]."|".$row[4]."|".$row[5]."|". utf8_encode($calle) ."%";
