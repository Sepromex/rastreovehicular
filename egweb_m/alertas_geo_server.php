...
function nuevo_folio($T_dias,$T_geo,$T_veh,$inicio,$fin,$minima,$maxima,$descrip,$correos,$dentro,$fuera,$periodo,$activo,$gestion,$duracion,$estricto){
	...
	$mail=$correos;
	$gas=0;
	$insert="INSERT INTO gpscondicionalerta VALUES(0,'$descrip',$estricto,".$sess->get('Ide').",'".date("Y-m-d H:i:s")."','$mail',0,0,$no_repetir,$activo,-1,0,0,0,-1)";
	mysql_query($insert);
	$f=mysql_fetch_array(mysql_query("SELECT folio FROM gps_config ORDER BY folio DESC LIMIT 0, 1"));
	$folio = $f[0]+1;
	switch($periodo){
		case 'sem':
			$dias='S:';
	...
	$insert_C="INSERT INTO SEPROMEX.gps_config VALUES($folio,$minima,$maxima,$dentro,$fuera,'$inicio','$fin','$dias','$geocercas',1,'$gestion')";
	mysql_query($insert_C);
	for($i=0;$i<count($T_veh);$i++){
		for($j=0;$j<count($T_geo);$j++){
			$insert_entrar="INSERT INTO SEPROMEX.gpscondicionalertadet 
			...
			$insert_salir="INSERT INTO SEPROMEX.gpscondicionalertadet 
				VALUES($folio,0,".$sess->get("Ide").",".$T_veh[$i].",0,$minima,$maxima,".$T_geo[$j].",0,$fuera,'$inicio','$fin',$duracion,0,1,
				$activo,'$dias','".date("Y-m-d H:i:s")."','-1',0,0,$gas);";
				mysql_query($insert_salir);
			$I = "INSERT INTO SEPROMEX.geo_veh values(".$T_veh[$i].",".$T_geo[$j].",0,1,1,'".date("Y-m-d H:i:s")."',".$sess->get('Idu').",0,1,1,1)";
				mysql_query($I);
			$ver = mysql_query("SELECT * from SEPROMEX.geo_veh where num_veh=".$T_veh[$i]." and num_geo=".$T_geo[$j]);
			if(mysql_num_rows($ver)==0){
				$I_ = "INSERT INTO SEPROMEX.geo_veh values(".$T_veh[$i].",".$T_geo[$j].",0,1,1,'".date("Y-m-d H:i:s")."',".$sess->get('Idu').",0,1,1,1)";
				mysql_query($I_);
			}
			else{
				$II = "UPDATE SEPROMEX.geo_veh SET activo=1 where num_veh=".$T_veh[$i]." and num_geo=".$T_geo[$j];
				mysql_query($II);
			}
		}
...
function config_folio($folio){
	...
	$query_chec="SELECT GPS.activo,GPS.descripcion,GPS.enviaremail,GPS.folio
			FROM sepromex.gpscondicionalerta GPS
			WHERE GPS.id_empresa=".$sess->get('Ide')."
			AND GPS.folio=$folio 
			and GPS.activo=1";
	...
	$correo=str_replace(";",'\n',$dat[2]);
	$query=mysql_query("SELECT * FROM sepromex.gps_config where folio=$folio");
	...
	$query_g=mysql_query("SELECT * FROM sepromex.gpscondicionalertadet where folio=$folio and activo=1 order by reg asc");
	$reglas="
	...
	while($row=mysql_fetch_array($query_g)){
		$vehiculo='';
		$msjxclave='';
		$query=mysql_query("SELECT id_veh from sepromex.vehiculos where num_veh=".$row[3]);
		...
		$horario='';
		if($row[4]!=''){
			$query=mysql_query("SELECT mensaje FROM sepromex.c_mensajes where id_mensaje=".$row[4]." and id_empresa=".$sess->get('Ide'));
			if(mysql_num_rows()==0){
				$query=mysql_query("SELECT mensaje FROM sepromex.c_mensajes where id_mensaje=".$row[4]." and id_empresa=15");
		...
		if($row[7]!=0){
			$query=mysql_query("SELECT nombre FROM sepromex.geo_time where id_empresa=".$sess->get('Ide')." AND num_geo=".$row[7]." and activo=1");
