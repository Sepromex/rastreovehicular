...
function solicitud($formSalida,$idu,$veh){
	...
	if($pass==''){
		...
		$cad_emp = "select username,id_empresa from sepromex.usuarios where id_usuario='$idu' and password='$pass'";//and password='$pass'
		//echo $cad_emp;
		...
		if($row[0]){
			...
			//pero el comando solo las activa o las desactiva
				...
				if($formSalida['tipo'] == 1){//activar
					...
					$consulta = "insert into auditabilidad values (0,'$idu','".date("Y-m-d H:i:s")."',57,'Activacion de salida digital ($veh) - $com',
				...
				if($formSalida['tipo'] == 0){//desactivar
					...
					$consulta = "insert into auditabilidad values (0,'$idu','".date("Y-m-d H:i:s")."',58,'Desactivaccion de salida digital ($veh) - $com',
					13,$row[1],'".get_real_ip()."')";
					mysql_query($consulta);
				}
				
				$socket = socket_create(AF_INET, SOCK_DGRAM, 0);
				$cere = socket_connect($socket,"10.0.2.8",'6668'); //depende de la ip en que se encuentre el cerebro modificado por rikardo rojas //6668 // 10.0.2.8
				$paq = "EMAIL:".$com;
