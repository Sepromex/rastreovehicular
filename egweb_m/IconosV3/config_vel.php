...
function velocidades(){
	...
	$idemp = $sess->get("Ide");
	$idu=$sess->get("Idu");
	$query="SELECT v.ID_VEH, v.NUM_VEH,v.estatus,ev.publicapos,
			ev.descripcion
			FROM veh_usr AS vu
	...
	while($row=mysql_fetch_array($rows)){
		...
		if(mysql_num_rows($ver)==1){
			...
			//$configurado.="<img src='img2/apply.png' width='15px' style='float:right;' title='Ya tienen configuraci&oacute;n' alt='configuraci&oacute;n'>";
		}
		else{
			$configurado="";
		}
...
function mostrar_config($veh_T){
	...
	if($total>=1){
		...
		if(mysql_num_rows($rows)==0 && $total>=1){
			...
			//$objResponse->assign("contenido","innerHTML","no hay");
