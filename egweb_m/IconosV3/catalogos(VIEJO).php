...
function detallar_datos($veh,$est)
{
	...
	if($resp_detalle != 0)
	{
		...
		$cad_dsn .= "<tr><td>Placas</td><td colspan='2'>".htmlentities($row_veh[3])."</td></tr>";
		//$cad_dsn .= "<tr><td>Color:</td><td colspan='2'>".htmlentities($row_veh[4])."</td></tr>";
		$cad_dsn .= "<tr><td>Modelo:</td><td colspan='2'>".htmlentities($row_veh[5])."</td></tr>";
		$cad_dsn .= "<tr><td>Detalle:</td><td colspan='2'>".htmlentities($row_veh[6])."</td></tr>";
	...
	switch($n){
		case 1: //$idu = $ide;
				...
				if($resp_veh != 0 ){
					...
					while($row = mysql_fetch_array($resp_veh))
					{
						if($cont == 0)
						{
							$cad_cont_veh .= "<option value='$row[1]' selected=true>$row[0]</option>";	
							$numveh = $row[1];
						}
						else $cad_cont_veh .= "<option value='$row[1]'>$row[0]</option>";	
						$cont++;
					}
				$cad_cont_veh .= "</select>";
		...
		case 2: if($est != 3){
				...
				if(mysql_num_rows($resp_sitios) != 0 ){
					...
					$tabla_sitios .= "<td>Nombre</td><td>Tipo</td><td>Lat</td><td>Long</td><td>Contacto</td><td>Tel</td>";
      				$tabla_sitios .= "<td>Tel</td><td>Mod</td><td>Elim</td></tr>";
					while($fila = mysql_fetch_array($resp_sitios)){
						...
						if($est != 3){
							$tabla_sitios .="<td><img src='img/ico_edita.png' width='15px' height='15px' ";
							$tabla_sitios .="border='0' title='Modificar sitio' onclick='nueva_ventana($ide,$fila[7])' /></td>";
							$tabla_sitios .="<td><img src='img/ico_delete.png' width='15px' height='15px' ";
		...
		case 3: $cad_sitios = "select num_geo,nombre,IF(tipo = 1,'POLIGONAL','CIRCULAR') AS tipo_geo,tipo from geo_time where id_empresa = $ide";
				...
				if(mysql_num_rows($resp_sitios) != 0 ){
					...
					$tabla_sitios .= "<td>Nombre</td><td width='120'>Tipo</td><td width='30'>Eliminar</td></tr>";
					while($fila = mysql_fetch_array($resp_sitios)){
