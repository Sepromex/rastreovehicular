...
function modificar_sitio($id_sitio){
	...
	$form="
	<table id='newspaper-a1' style='margin:0px;padding:0px;'>
		<tr>
...
function update_sitio($id_sitio,$nombre_sitio,$tipo_sitio,$contacto_sitio,$tel1,$tel2){
	...
	$ide=$sess->get("Ide");
	mysql_query("UPDATE sitios set 
	nombre='$nombre_sitio',
	id_tipo=$tipo_sitio,
	contacto='$contacto_sitio',
	tel1='$tel1',
	tel2='$tel2',
	activo=1
...
function ver_eventos($veh,$pag){
	...
	$objResponse = new xajaxResponse();
	$por_pag=$pag*15;
	$query=mysql_query("SELECT l.fechahora,l.evento,l.procesado
	from el_monitorgpslog l 
	inner join el_monitorgps m on l.num_veh=m.num_veh
	where l.num_veh=$veh 
	order by l.idlog desc limit $por_pag,15");
	$q_sig=mysql_query("SELECT l.fechahora,l.evento,l.procesado
	from el_monitorgpslog l 
	inner join el_monitorgps m on l.num_veh=m.num_veh
	where l.num_veh=$veh 
	order by l.idlog desc limit $por_pag,16");
...
function exportar($ide){
	$objResponse = new xajaxResponse();
	$iframe="
	<iframe src='exporta_sitios.php?ide=$ide'></iframe>";
...
function n_correo($veh){
	...
	$ver=mysql_query("Select d.folio,m.descripcion,m.enviaremail,d.duracion ,d.horaini,d.horafin,d.enlosdias
		...
		and d.vel_min=-1 
		and d.vel_max=-1 
		and d.id_msjxclave=0 
		and d.entrageo=0 
		and d.salegeo=0 
		and d.activo=1
		and m.presolicitapos=15
		and num_veh=$veh");
...
function agrega_correo($T_correo){
	...
	//$objResponse->alert("entra".$mostrar);
	return $objResponse;
}
...
function detallar_datos($veh,$est){
	$objResponse = new xajaxResponse();
	$cad_detalle = "select num_veh,id_veh,economico,placas,color,modelo,detalle,hora
					from vehiculos 
					where num_veh = $veh";
	...
	$cad_geo = "select t.nombre,v.num_geo 
				from geo_veh v 
				inner join geo_time t on(t.num_geo = v.num_geo) 
				where v.num_veh = $veh
				and v.activo=1";
	...
	if($resp_detalle != 0)
	{
		...
		$obsoleto=mysql_query("select distinct v.id_sistema, s.tipo_equipo
							from vehiculos v
							  inner join sistemas s on v.id_sistema=s.id_sistema
							where s.tipo_equipo not in
							  ('A1','SPIDER','CTRACKER','GALAXY','PDT','PORTMAN','SUNTECH','X8')
							and s.obsoleto=0
							and v.num_veh=$veh");
...
function guardarDatos($datos,$numveh){
	...
	if($numveh != "" && $numveh != null)
	{
		...
		//$update = "update vehiculos set modelo='$modelo',placas='$placas',detalle='$detalle',hora='$zona' where num_veh=$numveh";
...
function todos(){
	$objResponse = new xajaxResponse();
	$vehiculos=mysql_query("select v.num_veh,v.tpoleo from vehiculos v
	inner join estveh e on e.estatus=v.estatus
	inner join ultimapos u on v.num_veh=u.num_veh
	where e.publicapos=1
	and u.fecha>'2014-10-29 00:00:00'");
...
function monitorista($veh,$visto){
	...
	$query=mysql_query("SELECT distinct(v.id_veh),s.descripcion,v.tpoleo,u.fecha
	from vehiculos v
	inner join sistemas s on s.id_sistema=v.id_sistema
	inner join ultimapos u on u.num_veh=v.num_veh
	where v.num_veh=$veh
	and u.fecha>='$hora_atras'");
	$actualizado=0;
	if(mysql_num_rows($query)>0){
		$actualizado=1;
	}
	...
	//$boton_todos="<input type='button' value='todos' onclick='xajax_todos()'>";
...
function g_monitor($veh,$tiempo,$hrs,$correos,$activo,$ini,$fin,$T_dias,$notifica){
	...
	$options="";
	$sess =& patSession::singleton('ham', 'Native', $options );
	//$objResponse->alert($tiempo." - ".$correos." - ".$ini." - ".$fin." - ".$T_dias);
	$dias='S:';
	for($x=0;$x<count($T_dias);$x++){
		$dias.=$T_dias[$x];
		if($x<(count($T_dias)-1)){
	...
	$alarmado=0;//aun no se para que es
	switch($activo){
		case 0:
...
function g_tiempo_motor($veh,$tiempo,$correos,$activo,$folio,$ini,$fin,$T_dias){
	...
	switch($activo){
		...
		case 1://nuevo
			...
			and d.num_veh=$veh");
			/*if(mysql_num_rows($ver)==0){
				$descripcion="Tiempo encendido ($tiempo hr$plural)";
				$q="insert into gpscondicionalerta 
				values(0,'$descripcion',0,".$sess->get('Ide').",'".date("Y-m-d H:i:s")."','$correos',0,0,5,1,-1,0,0,0,-1)";
				mysql_query($q);
				$folio=mysql_insert_id();
				$minutos=$tiempo*60;
				$q2="insert into gpscondicionalertadet 
				values($folio,0,".$sess->get('Ide').",$veh,248,-1,-1,0,0,0,'$ini','$fin',$minutos,0,1,1,'$dias',
				'".date("Y-m-d H:i:s")."',-1,0,0)";
				mysql_query($q2);
				mysql_query("Insert into gps_config 
				values($folio,-1,-1,0,0,'$ini','$fin','$dias',0,1,'server')");
			}
			else{
				$dat=mysql_fetch_array($ver);
				$folio=$dat[0];
				$minutos=$tiempo*60;
				mysql_query("UPDATE gpscondicionalertadet set activo=1,duracion=$minutos,enlosdias='$dias',horaini='$ini',horafin='$fin' 
				where folio=$folio");
				$descripcion="Tiempo encendido ($tiempo hr$plural)";
				mysql_query("UPDATE gpscondicionalerta set activo=1,enviaremail='$correos',descripcion='$descripcion'
				where folio=$folio");
				mysql_query("UPDATE gps_config set horaini='$ini',horafin='$fin',enlosdia='$dias'
				where folio=$folio ");
			}*/
			$objResponse->assign("correos_varios_2","innerHTML","Datos guardados");
		break;
...
##PARTE DE USUARIOS _________________________________________________________________________________________________________________________________________
...
function crearInterUsr($idup,$ide){
	...
	$cad_veh = "
	SELECT distinct(v.NUM_VEH), v.ID_VEH FROM veh_usr AS vu 
	Inner Join vehiculos AS v ON vu.NUM_VEH = v.NUM_VEH 
	inner join estveh ev on (v.estatus = ev.estatus) 
	WHERE vu.ID_USUARIO = $idup 
	and vu.activo=1 
	AND ev.publicapos=1
	ORDER BY v.ID_VEH ASC";
...
function crearInterUsrMod($idup,$idu,$ide){
	$objResponse = new xajaxResponse();
		...
		$cad_veh = "SELECT distinct(v.NUM_VEH), v.ID_VEH FROM veh_usr AS vu Inner Join vehiculos AS v ON ";
   		$cad_veh .= "vu.NUM_VEH = v.NUM_VEH WHERE vu.ID_USUARIO = $idup and vu.activo=1 ORDER BY v.ID_VEH ASC";
   		$res_veh = mysql_query($cad_veh);
		while($row = mysql_fetch_array($res_veh)){
