...
function rep_gas($formu,$veh){
	...
	if ($veh == ''){
		...
		if(empty($fecha_ini)==0 && empty($fecha_fin)==0 && date($fecha_ini) < date($fecha_fin))
		{
			...
			$query="SELECT max(id_pos) from posiciones_test where fecha>='".date("Y-m-d",$ayer)." 23:59:00' 
					and fecha<='$fecha_ini';";
			...
			$hoy_u2=mysql_query("SELECT max(id_pos) from posiciones_test");
			...
			if(preg_match("/A/i",$entrada)){
				...
				inner join posiciones_test p on p.id_pos=t.id_pos
				...
				from pos_gas g 
				inner join posiciones_test p on p.id_pos=g.id_pos				
				where $max_pos p.num_veh = '$veh'  
...
function ultimo_encendido($id_pos,$ini,$veh){
	...
	*/
	...
	$m_248=mysql_query("SELECT max(id_pos) from posiciones_test where num_veh=$veh and id_pos>$anterior[0] 
	and fecha<'$ini' 
	and entradas=248");
	$m_249=mysql_query("SELECT max(id_pos) from posiciones_test where num_veh=$veh and id_pos>$anterior[0] 
	and fecha<'$ini'
	and entradas=249");
	if(mysql_num_rows($m_248)>0 || mysql_num_rows($m_249)>0){//si hay algun registro entramos
		...
		if(mysql_num_rows($m_248)>0 && mysql_num_rows($m_249)>0){//si existen los 2 registros
			...
			$fecha_248=mysql_query("SELECT fecha from posiciones_test where id_pos=".$dat_248[0]);
			$fecha_249=mysql_query("SELECT fecha from posiciones_test where id_pos=".$dat_249[0]);
		...
		*/
		...
		if(!empty($fecha_ini) && !empty($fecha_fin) && date($fecha_ini) < date($fecha_fin)){
			...
			if($fecha_ini > "2015-03-28"){
				...
				//$query="SELECT max(id_pos) as max_idpos from posiciones_test where fecha>='".date("Y-m-d",$ayer)." 23:59:00' and fecha<='$fecha_ini'";	
				//echo "||"."SELECT max(id_pos) as max_idpos from posiciones_test where fecha>='".date("Y-m-d",$ayer)." 23:59:00'  and fecha<='$fecha_ini'".""	
				//$ultimas=mysql_query($query);					
				//$ultima=mysql_fetch_array($ultimas);						
				...
				$datos_rec =
					...
					from sepromex.posiciones_test p
...
function recorrido_masivo($valorForm,$id_pos,$total){
	...
	if(date("Y-m-d")==$d_hoy[0]){
		...
		if(date("Y-m-d")!=date("Y-m-d",$dia_m)){
			...
			$manana=mysql_query("SELECT max(id_pos) from posiciones_test",$conec);
		}
